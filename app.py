...
# Set page config
st.set_page_config(
    page_title="BFF fossil generation heatmap",
    page_icon=":bar_chart:",
    layout="wide"
)
...
# Country filter
available_countries = ['Europe'] + sorted([country for country in df['Country'].unique() if country not in (['Europe','United Kingdom'])])
...
selected_fuel = st.sidebar.radio(
    "Select the type of fossil fuel:",
...
fig.add_trace(go.Heatmap(
    ...
    colorbar=dict(title="Number of hours"),
    hoverongaps=False,
...
fig.update_layout(
    title=f'<i>{selected_country}</i>: Number of hours when <i>{selected_fuel.lower()}</i> represented <i>{selected_share_bin}</i> of total generation', 
    title_x=0.1,
    xaxis_title='Month',
    yaxis_title='Year',
    width=800,
...
if selected_country == 'Europe':
    st.markdown('⚠️ "Europe" represents the average across all listed countries, excluding Italy due to data quality concerns, and Ukraine dut to missing data. It shows the mean number of hours when generation from the selected fuel source fell below the specified percentage threshold.')
